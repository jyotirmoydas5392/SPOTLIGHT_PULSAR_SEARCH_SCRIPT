...
def process_files_for_gpu(node_alias, gpu_id, input_file_dir, output_dir, log_dir, file_processing_delay, command_template):
    """
    Process files sequentially on the given GPU of the given node.
    """
    files_to_process = [
        f for f in os.listdir(input_file_dir)
        if f.startswith("AA_2dhs") and f.endswith(f"node_{node_alias}_gpu_id_{gpu_id}.txt")
    ]
...
def process_node(node_alias, input_file_dir, output_dir, log_dir, gpu_0_start_delay, gpu_1_start_delay, file_processing_delay, command_template):
    """
    ...
    p_gpu_0 = Process(
        ...
        args=(node_alias, 0, input_file_dir, output_dir, log_dir, file_processing_delay, command_template)
    )
    ...
    p_gpu_1 = Process(
        target=process_files_for_gpu,
        args=(node_alias, 1, input_file_dir, output_dir, log_dir, file_processing_delay, command_template)
    )
